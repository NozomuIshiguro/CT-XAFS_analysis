...
__kernel void XANES_fitting(__read_only image2d_array_t mt_img,
                            ...
                            __constant float *energy,__global float *lambda,__global float *rho,
                            ...
                            __constant float *para_upper_limit)
{
    ...
    for(int en=0; en<ENERGY_NUM; en++){
        //mt_fit (trial)
        float J[PARA_NUM];
        float dF, mt_fit, mt_data;
        mt_data = read_imagef(mt_img,s_linear,(float4)(global_x,global_y,en,0)).x;
        FIT(energy[en],mt_fit,fp);
        JACOBIAN(energy[en],J,fp);
    ...
    for(int i=0; i<PARA_NUM; i++){
        ...
        fp[i] += dp[i];
        fp[i] = (fp[i]<para_lower_limit[i]) ? para_lower_limit[i]:fp[i];
        fp[i] = (fp[i]>para_upper_limit[i]) ? para_upper_limit[i]:fp[i];
    ...
    rho[global_ID] = (chi2_old - chi2_new)/d_L;
