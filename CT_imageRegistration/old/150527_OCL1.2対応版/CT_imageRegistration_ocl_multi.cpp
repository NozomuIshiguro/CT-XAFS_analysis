...
int mt_output_thread(int startAngleNo, int EndAngleNo, int startEnergyNo, int endEnergyNo,
                     string output_dir, vector<float*> mt_outputs,
                     vector<float*> transpara,vector<float*> transpara_err,
                     int waittime){
    ...
    for (int j=startAngleNo; j<=EndAngleNo; j++) {
        ...
        for (int i=startEnergyNo; i<=endEnergyNo; i++) {
            ...
            for (int k=0; k<transpara_num; k++) {
                ofs<<transpara[i-startEnergyNo][k+transpara_num*(j-startAngleNo)]<<"\t"
                <<transpara_err[i-startEnergyNo][k+transpara_num*(j-startAngleNo)]<<"\t";
            }
            ofs<<endl;
        }
        ofs.close();
    ...
    for (int i=startEnergyNo; i<=endEnergyNo; i++) {
        ...
        delete [] transpara[i-startEnergyNo];
        delete [] transpara_err[i-startEnergyNo];
    }
    return 0;
}
...
int imageReg_thread(cl::CommandQueue command_queue, vector<cl::Kernel> kernel,
    ...
    try {
        ...
        float *transpara;
        float *transpara_err;
        transpara=new float[IMAGE_SIZE_M*dA];
        transpara_err=new float[IMAGE_SIZE_M*dA];
        vector<float*>transpara_vec;
        vector<float*>transpara_err_vec;
        for (int i=startEnergyNo; i<=endEnergyNo; i++) {
            sample_img.push_back(new float[IMAGE_SIZE_M*dA]);
            transpara_vec.push_back(new float[transpara_num*dA]);
            transpara_err_vec.push_back(new float[transpara_num*dA]);
        ...
        vector<cl::Image2DArray> mt_target_images;
        vector<cl::Image2DArray> mt_sample_images;
        ...
        for (int k=0; k<num_buf; k++) {
			...
			It_target_buffers.push_back(cl::Buffer(context, CL_MEM_READ_WRITE, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, 0, NULL));
			I0_sample_buffers.push_back(cl::Buffer(context, CL_MEM_READ_ONLY, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, 0, NULL));
			It_sample_buffers.push_back(cl::Buffer(context, CL_MEM_READ_WRITE, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, 0, NULL));
			mt_output_buffers.push_back(cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, 0, NULL));
            mt_target_images.push_back(cl::Image2DArray(context, CL_MEM_READ_WRITE,
                                                        format, buf_pnts, IMAGE_SIZE_X,
                                                        IMAGE_SIZE_Y,0,0,NULL,NULL));
            mt_sample_images.push_back(cl::Image2DArray(context, CL_MEM_READ_WRITE,
                                                        format, buf_pnts, IMAGE_SIZE_X,
                                                        IMAGE_SIZE_Y,0,0,NULL,NULL));
        }
        ...
        for (int k=0; k<num_buf; k++) {
            command_queue.enqueueWriteBuffer(It_target_buffers[k], CL_TRUE, 0, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, It_img_target+IMAGE_SIZE_M*buf_pnts*k,NULL,NULL);
            command_queue.finish();
        }
        t=0;
        for (int k=0; k<num_buf; k++) {
        ...
        //t++;
        for (int k=0; k<num_buf; k++) {
            kernel[1].setArg(t, It_target_buffers[k]);
            t++;
        }
        //kernel[1].setArg(t, cl::Local(sizeof(cl_float*)*dA));
        ...
        //t++;
        for (int k=0; k<num_buf; k++) {
            kernel[2].setArg(t, It_target_buffers[k]);
            t++;
        }
        for (int k=0; k<num_buf; k++) {
            kernel[2].setArg(t, mt_target_images[k]);
            t++;
        }
        //kernel[2].setArg(t, cl::Local(sizeof(cl_float*)*dA));
        ...
        if ((targetEnergyNo>=startEnergyNo)&(targetEnergyNo<=endEnergyNo)) {
            for (int k=0; k<num_buf; k++) {
				command_queue.enqueueReadBuffer(It_target_buffers[k], CL_TRUE, 0, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, sample_img[targetEnergyNo - startEnergyNo] + IMAGE_SIZE_M*buf_pnts*k, NULL, NULL);
                command_queue.finish();
            }
            ...
            for (int j=startAngleNo; j<=EndAngleNo; j++) {
                ...
                oss <<OSS_TARGET;
                for (int t=0; t<transpara_num; t++) {
                    transpara_vec[targetEnergyNo - startEnergyNo][t+transpara_num*(j-startAngleNo)]=0;
                    transpara_err_vec[targetEnergyNo - startEnergyNo][t+transpara_num*(j-startAngleNo)]=0;
                }
            }
            cout << oss.str();
        ...
        for (int s=0; s<2; s++) {//1st cycle:i<targetEnergyNo, 2nd cycle:i>targetEnergyNo
            ...
            for (int i=LoopStartenergyNo[s]+di; i*di<=energyNo[s]*di; i+=di) {
                ...
                for (int k=0; k<num_buf; k++) {
                    command_queue.enqueueWriteBuffer(It_sample_buffers[k], CL_TRUE, 0, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, sample_img[i-startEnergyNo]+IMAGE_SIZE_M*buf_pnts*k);
                    command_queue.finish();
                }
                t=0;
                for (int k=0; k<num_buf; k++) {
                ...
                //t++;
                for (int k=0; k<num_buf; k++) {
                    kernel[4].setArg(t, It_sample_buffers[k]);
                    t++;
                }
                //kernel[4].setArg(t, cl::Local(sizeof(cl_float*)*dA));
                ...
                //t++;
                for (int k=0; k<num_buf; k++) {
                    kernel[5].setArg(t, It_sample_buffers[k]);
                    t++;
                }
                for (int k=0; k<num_buf; k++) {
                    kernel[5].setArg(t, mt_sample_images[k]);
                    t++;
                }
                //kernel[5].setArg(t, cl::Local(sizeof(cl_float*)*dA));
                ...
                t=0;
                for (int k=0; k<num_buf; k++) {
                    kernel[6].setArg(t, mt_target_images[k]);
                    t++;
                }
                //kernel[6].setArg(t, cl::Local(sizeof(cl_float*)*dA));
                //t++;
                for (int k=0; k<num_buf; k++) {
                    kernel[6].setArg(t, mt_sample_images[k]);
                    t++;
                }
                //kernel[6].setArg(t, cl::Local(sizeof(cl_float*)*dA));
				//t++;
                ...
                for (int k=0; k<dA; k++) {
                    ...
                    for (int n=0; n<transpara_num; n++) {
                        if (log10(transpara_err[n+k*transpara_num])>0) {
                            precision_err[n]=1;
                        }else{
                            precision_err[n]=(floor(log10(abs(transpara[n+k*transpara_num])))
                                              -floor(log10(transpara_err[n+k*transpara_num]))+1);
                        }
                    }
                    ...
                    for (int n=0; n<transpara_num; n++) {
                        transpara_vec[i - startEnergyNo][n+transpara_num*k]=transpara[n+k*transpara_num];
                        transpara_err_vec[i - startEnergyNo][n+transpara_num*k]=transpara_err[n+k*transpara_num];
                    }
                }
                cout << oss.str();
        ...
        delete [] transpara;
        delete [] transpara_err;
        
        int delta_t;
        if (last) {
            delta_t=0;
        }else{
            delta_t= max(60.0,difftime(readfinish_t,start_t)*0.4);
        ...
        thread th_output(mt_output_thread,
                         ...
                         output_dir,sample_img,transpara_vec,transpara_err_vec,delta_t);
                          ...
                          OCL_platform_device plat_dev_list)
{
    ...
    ifs3.close();
     //kernel preprocessor source (XY) from cl file
     ifstream ifs4("./XYshift_scale.cl",ios::in);
     if(!ifs4) {
     cerr << "   Failed to load kernel \n\n" << endl;
     return -1;
     }
