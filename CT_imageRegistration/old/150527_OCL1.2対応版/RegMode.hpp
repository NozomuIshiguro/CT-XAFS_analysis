...
#define reductpara_num 15

#define OSS_TARGET "    registration shift: dx=0, dy=0, d(theta)=0, scale=1.0\n\n"
...
oss<<((float*)(tp))[1]<<" +/- ";\
oss.precision(1);\
oss<<abs(((float*)(tp_err))[1])<<", d(theta)=";\
...
#define OFS_TRANSPARA(ofs) ofs<<"dx\tdx error\tdy\tdy error\td(theta)\t";\
ofs<<"d(theta) error\td(scale)\td(scale) error"<<endl;

#elif REGMODE==2  //scale+xy shift

#define transpara_num 3
#define reductpara_num 10
...
#define OFS_TRANSPARA(ofs) ofs<<"dx\tdx error\tdy\tdy error\td(scale)\td(scale) error"<<endl;
...
#define OSS_TARGET "    registration shift: dx=0, dy=0, d(theta)=0\n\n"
...
oss<<((float*)(tp))[0]<<" +/- ";\
oss.precision(1);\
oss<<((float*)(tp_err))[0]<<", dy=";\
oss.precision(((int*)(p_err))[1]);\
oss<<((float*)(tp))[1]<<" +/- ";\
oss.precision(1);\
oss<<((float*)(tp_err))[1]<<", d(theta)=";\
oss.precision(((int*)(p_err))[2]);\
oss<<((float*)(tp))[2]<<" +/- ";\
oss.precision(1);\
oss<<((float*)(tp_err))[2]<<"\n\n";\
...
#define OFS_TRANSPARA(ofs) ofs<<"dx\tdx error\tdy\tdy error\td(theta)\td(theta) error"<<endl;

#else //xy shift

#define transpara_num 2
#define reductpara_num 6
...
#define OFS_TRANSPARA(ofs) ofs<<"dx\tdx error\tdy\tdy error"<<endl;
