...
int imageReg_thread(cl::CommandQueue command_queue, vector<cl::Kernel> kernel,
    ...
    try {
        ...
        //Buffer declaration
		cl::Buffer dark_buffer(context, CL_MEM_READ_ONLY, sizeof(cl_float)*IMAGE_SIZE_M*dA, 0, NULL);
		cl::Buffer I0_target_buffer(context, CL_MEM_READ_ONLY, sizeof(cl_float)*IMAGE_SIZE_M*dA, 0, NULL);
		cl::Buffer It2mt_target_buffer(context, CL_MEM_READ_WRITE, sizeof(cl_float)*IMAGE_SIZE_M*dA, 0, NULL);
		cl::Buffer I0_sample_buffer(context, CL_MEM_READ_ONLY, sizeof(cl_float)*IMAGE_SIZE_M*dA, 0, NULL);
        ...
        cl::Image2DArray mt_target_outputImg(context, CL_MEM_READ_WRITE,format,dA,
                         IMAGE_SIZE_X,IMAGE_SIZE_Y,
                         0,0,NULL,NULL);
        cl::Image2DArray mt_sample_outputImg(context, CL_MEM_READ_WRITE,format,dA,
                                          IMAGE_SIZE_X,IMAGE_SIZE_Y,
                                          0,0,NULL,NULL);
		...
		//write dark buffer
		for (int k = 0; k<dA; k++) {
			command_queue.enqueueWriteBuffer(dark_buffer, CL_TRUE, sizeof(cl_float)*IMAGE_SIZE_M*k, sizeof(cl_float)*IMAGE_SIZE_M, dark_img, NULL, NULL);
		...
		for (int k = 0; k<dA; k++) {
			command_queue.enqueueWriteBuffer(I0_target_buffer, CL_TRUE, sizeof(cl_float)*IMAGE_SIZE_M*k, sizeof(cl_float)*IMAGE_SIZE_M, I0_img_target, NULL, NULL);
		}
        ...
        for (int s=0; s<2; s++) {//1st cycle:i<targetEnergyNo, 2nd cycle:i>targetEnergyNo
            ...
            for (int i=LoopStartenergyNo[s]+ds; i*di<=LoopEndenergyNo[s]*di; i+=di) {
                ...
                for (int k = 0; k<dA; k++) {
                    command_queue.enqueueWriteBuffer(I0_sample_buffer, CL_TRUE, sizeof(cl_float)*IMAGE_SIZE_M*k, sizeof(cl_float)*IMAGE_SIZE_M, I0_imgs[i-startEnergyNo], NULL, NULL);
                }
                          ...
                          OCL_platform_device plat_dev_list, regMode regmode)
{
    ...
    for (int i=0; i<plat_dev_list.size(); i++) {
        ...
        dA.push_back(4);
        cout<<"Number of working compute unit: "<<dA[i]<<endl<<endl;
