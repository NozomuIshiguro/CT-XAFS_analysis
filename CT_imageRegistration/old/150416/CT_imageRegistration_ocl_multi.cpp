    ...
    for (int j=startAngleNo; j<=EndAngleNo; j++) {
        ...
        for (int i=startEnergyNo; i<=endEnergyNo; i++) {
            ...
            for (int k=0; k<transpara_num; k++) {
                ofs<<transpara[i-startEnergyNo+offsetEnergyNo][k+transpara_num*(j-startAngleNo)]<<"\t"
                <<transpara_err[i-startEnergyNo+offsetEnergyNo][k+transpara_num*(j-startAngleNo)]<<"\t";
            }
            ofs<<endl;
        }
        ofs.close();
...
int imageReg_thread(cl::CommandQueue command_queue, vector<cl::Kernel> kernel,
					...
					string fileName_base, string output_dir, int float_vec_size, bool last){
    try {
        ...
        if ((targetEnergyNo>=startEnergyNo)&(targetEnergyNo<=endEnergyNo)) {
            ...
            for (int k=0; k<num_buf; k++) {
				command_queue.enqueueReadBuffer(It2mt_target_buffers[k], CL_TRUE, 0, sizeof(cl_float)*IMAGE_SIZE_M*buf_pnts, sample_img[0] + IMAGE_SIZE_M*buf_pnts*k, NULL, NULL);
                command_queue.finish();
            }
            ...
            thread th_output(mt_output_thread,
                             startAngleNo,EndAngleNo,targetEnergyNo,targetEnergyNo,startEnergyNo,
                             output_dir,sample_img,transpara_vec,transpara_err_vec,0);
            th_output.detach();
        }
                          ...
                          OCL_platform_device plat_dev_list)
{
    ...
    ifs3.close();
     ...
     string kernel_src_XYrot(it4,last4);
     ifs4.close();*/
    ...
    int64_t RAM_size;
    GETTOTALSYSTEMMEMORY(RAM_size);
    cout<<RAM_size/1024/1024/1024<<" GB"<<endl;
    int64_t mem_usage=0;
    int float_vec_size=(endEnergyNo-startEnergyNo+1);
    do {
        for (int i=0; i<plat_dev_list.size(); i++) {
            mem_usage+=IMAGE_SIZE_M*sizeof(float)*(float_vec_size+1)*dA[i];
            //sample_img(dA*endEnergyNo-startEnergyNo+1), It_target(dA)
        }
        mem_usage+=IMAGE_SIZE_M*sizeof(float)*(endEnergyNo-startEnergyNo+3);
        //I0_sample(endEnergyNo-startEnergyNo+1), I0_target(1), dark(1)
        if (mem_usage<RAM_size) {
            break;
        }else{
            float_vec_size/=2;
        }
    } while (1);
    cout<<float_vec_size<<endl;
		...
		for (int j = 0; j<plat_dev_list.size(); j++) {
			...
			if (th.size()<plat_dev_list.size()) {
                ...
                th.push_back(thread(imageReg_thread, plat_dev_list.queue(j), kernels[j],
									...
									fileName_base, inp.getOutputDir(),
                                    float_vec_size,last));
