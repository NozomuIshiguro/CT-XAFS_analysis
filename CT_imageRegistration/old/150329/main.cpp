...
int main(int argc, const char * argv[]) {
    ...
    if (inp.getOutputDir().length()==0) {
            inp.setOutputDir("Set input raw his file directory.");
    }else{
    ...
    if (inp.getOutputDir().length()==0) {
        inp.setOutputDir("Set output file directory.");
    }else{
    ...
    if ((inp.getStartEnergyNo()==NAN)|(inp.getEndEnergyNo()==NAN)) {
        inp.setEnergyNoRange("Set energy num range (ex. 1-100).");
    }else{
    ...
    if (inp.getTargetEnergyNo()==NAN) {
        inp.setTargetEnergyNo("Set reference energy No. for image registration.");
    }else{
    ...
    if ((inp.getStartAngleNo()==NAN)|(inp.getEndAngleNo()==NAN)) {
        inp.setAngleRange("Set angle num range (ex. 1-1600).");
    }else{
