  ...
  dispatch_once(&once,
    ^{ int err = gclBuildProgramBinaryAPPLE("OpenCL/imageregistration_kernel_src.cl", "", &bmap, build_opts);
       if (!err) {
          ...
          assert(bmap.map[3].block_ptr == imageReg1X_kernel && "mismatch block");
          bmap.map[3].kernel = clCreateKernel(bmap.program, "imageReg1X", &err);
          assert(bmap.map[4].block_ptr == imageReg1Y_kernel && "mismatch block");
          bmap.map[4].kernel = clCreateKernel(bmap.program, "imageReg1Y", &err);
          assert(bmap.map[5].block_ptr == imageReg2X_kernel && "mismatch block");
          bmap.map[5].kernel = clCreateKernel(bmap.program, "imageReg2X", &err);
          assert(bmap.map[6].block_ptr == imageReg2Y_kernel && "mismatch block");
          bmap.map[6].kernel = clCreateKernel(bmap.program, "imageReg2Y", &err);
          assert(bmap.map[7].block_ptr == estimateParaError_kernel && "mismatch block");
          bmap.map[7].kernel = clCreateKernel(bmap.program, "estimateParaError", &err);
          assert(bmap.map[8].block_ptr == output_imgReg_result_kernel && "mismatch block");
          bmap.map[8].kernel = clCreateKernel(bmap.program, "output_imgReg_result", &err);
          assert(bmap.map[9].block_ptr == merge_mt_kernel && "mismatch block");
          bmap.map[9].kernel = clCreateKernel(bmap.program, "merge_mt", &err);
          assert(bmap.map[10].block_ptr == merge_rawhisdata_kernel && "mismatch block");
          bmap.map[10].kernel = clCreateKernel(bmap.program, "merge_rawhisdata", &err);
          assert(bmap.map[11].block_ptr == imQXAFS_smoothing_kernel && "mismatch block");
          bmap.map[11].kernel = clCreateKernel(bmap.program, "imQXAFS_smoothing", &err);
          assert(bmap.map[12].block_ptr == updateWeight_kernel && "mismatch block");
          bmap.map[12].kernel = clCreateKernel(bmap.program, "updateWeight", &err);
       }
     });
}
...
static void RegisterMap(void) {
  ...
  bmap.map[3].block_ptr = imageReg1X_kernel;
  bmap.map[4].block_ptr = imageReg1Y_kernel;
  bmap.map[5].block_ptr = imageReg2X_kernel;
  bmap.map[6].block_ptr = imageReg2Y_kernel;
  bmap.map[7].block_ptr = estimateParaError_kernel;
  bmap.map[8].block_ptr = output_imgReg_result_kernel;
  bmap.map[9].block_ptr = merge_mt_kernel;
  bmap.map[10].block_ptr = merge_rawhisdata_kernel;
  bmap.map[11].block_ptr = imQXAFS_smoothing_kernel;
  bmap.map[12].block_ptr = updateWeight_kernel;
}
