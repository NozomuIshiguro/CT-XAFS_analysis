...
#include "imageregistration_kernel_src_old.cl.h"
...
static void initBlocks(void) {
  ...
  dispatch_once(&once,
    ^{ int err = gclBuildProgramBinaryAPPLE("OpenCL/imageregistration_kernel_src_old.cl", "", &bmap, build_opts);
       if (!err) {
          ...
          assert(bmap.map[7].block_ptr == imQXAFS_smoothing_kernel && "mismatch block");
          bmap.map[7].kernel = clCreateKernel(bmap.program, "imQXAFS_smoothing", &err);
       }
     });
}
...
static void RegisterMap(void) {
  ...
  bmap.map[7].block_ptr = imQXAFS_smoothing_kernel;
}
