...
#include "imageregistration_kernel_src2.cl.h"

static void initBlocks(void);

// Initialize static data structures
static block_kernel_pair pair_map[9] = {
      { NULL, NULL },
      { NULL, NULL },
      { NULL, NULL },
      { NULL, NULL },
...
static block_kernel_map bmap = { 0, 9, initBlocks, pair_map };
...
void (^imageRegistration1_kernel)(const cl_ndrange *ndrange, cl_image mt_t_img, cl_image mt_s_img, cl_float* p, cl_float* p_target, cl_float* dF2, cl_float* tJdF, cl_float* tJJ, cl_int mergeN, size_t loc_mem) =
^(const cl_ndrange *ndrange, cl_image mt_t_img, cl_image mt_s_img, cl_float* p, cl_float* p_target, cl_float* dF2, cl_float* tJdF, cl_float* tJJ, cl_int mergeN, size_t loc_mem) {
  int err = 0;
  ...
  if (!k)
    gcl_log_fatal("kernel imageRegistration1 does not exist for device");
  kargs_struct kargs;
  gclCreateArgsAPPLE(k, &kargs);
  ...
  err |= gclSetKernelArgAPPLE(k, 7, sizeof(mergeN), &mergeN, &kargs);
  err |= gclSetKernelArgAPPLE(k, 8, loc_mem, NULL, &kargs);
  gcl_log_cl_fatal(err, "setting argument for imageRegistration1 failed");
  err = gclExecKernelAPPLE(k, ndrange, &kargs);
  gcl_log_cl_fatal(err, "Executing imageRegistration1 failed");
  gclDeleteArgsAPPLE(k, &kargs);
};

void (^imageRegistration2_kernel)(const cl_ndrange *ndrange, cl_image mt_t_img, cl_image mt_s_img, cl_float* p, cl_float* p_target, cl_float* dF2, cl_int mergeN, size_t loc_mem) =
^(const cl_ndrange *ndrange, cl_image mt_t_img, cl_image mt_s_img, cl_float* p, cl_float* p_target, cl_float* dF2, cl_int mergeN, size_t loc_mem) {
  int err = 0;
  ...
  if (!k)
    gcl_log_fatal("kernel imageRegistration2 does not exist for device");
  kargs_struct kargs;
  gclCreateArgsAPPLE(k, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 0, mt_t_img, &kargs);
  err |= gclSetKernelArgMemAPPLE(k, 1, mt_s_img, &kargs);
  ...
  gcl_log_cl_fatal(err, "setting argument for imageRegistration2 failed");
  err = gclExecKernelAPPLE(k, ndrange, &kargs);
  gcl_log_cl_fatal(err, "Executing imageRegistration2 failed");
  gclDeleteArgsAPPLE(k, &kargs);
};

void (^estimateParaError_kernel)(const cl_ndrange *ndrange, cl_float* p_error, cl_float* tJJ) =
^(const cl_ndrange *ndrange, cl_float* p_error, cl_float* tJJ) {
  int err = 0;
...
static void initBlocks(void) {
  ...
  dispatch_once(&once,
    ^{ int err = gclBuildProgramBinaryAPPLE("OpenCL/imageregistration_kernel_src2.cl", "", &bmap, build_opts);
       if (!err) {
          ...
          assert(bmap.map[3].block_ptr == imageRegistration1_kernel && "mismatch block");
          bmap.map[3].kernel = clCreateKernel(bmap.program, "imageRegistration1", &err);
          assert(bmap.map[4].block_ptr == imageRegistration2_kernel && "mismatch block");
          bmap.map[4].kernel = clCreateKernel(bmap.program, "imageRegistration2", &err);
          ...
          assert(bmap.map[6].block_ptr == merge_mt_kernel && "mismatch block");
          bmap.map[6].kernel = clCreateKernel(bmap.program, "merge_mt", &err);
          assert(bmap.map[7].block_ptr == merge_rawhisdata_kernel && "mismatch block");
          bmap.map[7].kernel = clCreateKernel(bmap.program, "merge_rawhisdata", &err);
          assert(bmap.map[8].block_ptr == imQXAFS_smoothing_kernel && "mismatch block");
          bmap.map[8].kernel = clCreateKernel(bmap.program, "imQXAFS_smoothing", &err);
       }
     });
}
...
static void RegisterMap(void) {
  ...
  bmap.map[3].block_ptr = imageRegistration1_kernel;
  bmap.map[4].block_ptr = imageRegistration2_kernel;
  bmap.map[5].block_ptr = estimateParaError_kernel;
  bmap.map[6].block_ptr = merge_mt_kernel;
  bmap.map[7].block_ptr = merge_rawhisdata_kernel;
  bmap.map[8].block_ptr = imQXAFS_smoothing_kernel;
}
