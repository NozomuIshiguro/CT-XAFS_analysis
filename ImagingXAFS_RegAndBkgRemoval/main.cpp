...
int main(int argc, const char * argv[]) {
    ...
    for(dp=readdir(dir);dp!=NULL;dp=readdir(dir)){
        ...
        if (darkname.find("_1.raw")!=string::npos) {
            cout << "raw file found: " << darkname <<endl<<endl;
            fileName_base += darkname;
            fileName_base.erase(fileName_base.size()-6);
            break;
        }
    }
    if (dp==NULL) {
    ...
    /*Image Registration*/
    imXAFS_RegAndBkgRemoval_ocl(fileName_base,inp,plat_dev_list,regmode);
